...
const StudentAttendanceHistoryModal: React.FC<StudentAttendanceHistoryModalProps> = ({ isOpen, onClose, student, records }) => {
  ...
  const handlePrint = () => {
    ...
    generatePdfWithBranding(
        ...
        {
            head: [['Date', 'Status']],
            body: studentRecords.map(rec => [rec.date, rec.status]),
        },
        `Attendance_History_${student.name.replace(/\s/g, '_')}.pdf`
    );
  };
      ...
      <div className="w-full max-w-lg p-6 bg-white rounded-lg shadow-xl dark:bg-gray-800" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-start mb-4">
            <div>
                ...
                <p className="text-gray-600 dark:text-gray-400">{student.name} ({student.studentId})</p>
            </div>

...
type NewStudentData = Omit<Student, 'id' | 'studentId' | 'avatarUrl' | 'dob' | 'address'> & { photo?: string };

interface CreateStudentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCreate: (student: NewStudentData) => void;
}

// FIX: Add optional 'name' prop to InputField props to match its usage.
const InputField: React.FC<{ id: string; name?: string; label: string; type?: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; required?: boolean; placeholder?: string; }> = 
...
const CreateStudentModal: React.FC<CreateStudentModalProps> = ({ isOpen, onClose, onCreate }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const studentData = {
            ...
            monthlyScholarship: parseFloat(formData.monthlyScholarship) || 0,
            photo: photo || undefined,
        };
        onCreate(studentData);
        // Reset form
        setFormData({ name: '', birthCertificateNo: '', parentName: '', parentPhone: '', guardianName: '', guardianPhone: '', class: '10', section: 'A', classRoll: '', monthlyScholarship: '' });
        setPhoto(null);
            ...
            <div className="w-full max-w-2xl p-6 bg-white rounded-lg shadow-xl dark:bg-gray-800" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="max-h-[70vh] overflow-y-auto pr-2">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="md:col-span-2">
                             ...
                             <div className="mt-1 flex items-center space-x-4">
                                ...
                                <label htmlFor="photo-upload" className="cursor-pointer bg-white dark:bg-gray-700 py-2 px-3 border border-gray-300 dark:border-gray-600 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <div className="flex justify-end mt-8 space-x-3 border-t dark:border-gray-600 pt-4">
                        ...
                        <button type="submit" className="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-md hover:bg-primary-700 focus:outline-none">Create Student</button>
                    </div>
                </form>
            </div>
        </div>
